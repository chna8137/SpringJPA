...
public class NoticeJoinEntity {
     ...
     * UserInfoEntity, NoticeJoinEntity 둘다 동일한 이름의 user_id 사용하고 있음
     * 둘 중 하나는 데이터 값의 변화를 주지 않기 위해 insertable = false, upatable = false 설정
     * insertalbe = false 컬럼에 등록 막기
     * updatable = flase 컬럼에 수정 막기
     * <p>
     *  변수 타입에 UserInfoEntity 정의하면, 자동으로 User_info 테이블이 조인 대상이 됨
     *  외래키는 PK값과 연결되는 구조이기에 user_info 테이블의 PK 컬럼인 user_id가 조인 대상이 됨
     *  즉, user_info 테이블의 user_id 컬럼과 notice 테이블의 user_id 컬럼이 조인됨
     * </p>
     * 만약, user_info 테이블의 PK 컬럼이 아닌 user_name과 같이 다른 컬럼을 사용하고 싶은 경우
